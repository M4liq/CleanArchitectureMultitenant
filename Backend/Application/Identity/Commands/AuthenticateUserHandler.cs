...
public static class AuthenticateUser
{
    ...
    public class Validator : IDomainValidationHandler<AuthenticateUserCommand>
    {
        ...
        {
            var userExists = await _dataContext.Set<IdentityEntity>().AnyAsync(user => user.Email == request.Email, cancellationToken: ct);
    ...
    public class Handler : IRequestHandler<AuthenticateUserCommand, AuthenticateUserResponse>
    {
        ...
        {
            var user = await _dataContext.Set<IdentityEntity>().FirstAsync(user => user.Email == request.Email, cancellationToken: ct);
            ...
            var claims = new List<Claim>
            {
                ...
                new(CustomClaimTypes.IdentityId, user.Id.ToString()),
                new (CustomClaimTypes.IdentityType, IdentityType.UserIdentity.ToString()),
                new(CustomClaimTypes.TenantId, user.TenantId.ToString())
            };
            
            var authenticationToken = await _authenticationTokenService.GenerateTokenAsync(new ClaimsIdentity(claims), ct: ct);
